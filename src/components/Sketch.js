...
export default class Sketch extends Component {
  ...
  async add () {
    try {
      ...
      const sketchWidth = Math.min(layout.window.width, this.sketch.renderer.width)
  ...
  onChangeAsync = async (edited) => {
    try {
      ...
      this.props.setAnnotatedMedia(snapshot, edited, layers, this.props.width, this.props.height)
    }
    catch (e) {
      api.postToSlack(JSON.stringify(e), 'Error')
    }
  ...
  render() {
    ...
    return (
      <View style={styles.container} >
        <View style={styles.container} >
          <View style={[ styles.sketchContainer, { width, height } ]} ref={ref => this.snapshotContainer = ref}>
              <ExpoPixi.Sketch
              ...
              <Image source={{ uri }} style={[ styles.bg_image, { width, height } ]}/>
          </View>
        </View>
      </View>
    );
