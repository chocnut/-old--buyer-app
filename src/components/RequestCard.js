...
export default class RequestCard extends React.Component {
  renderImage = (featured_image, sent_at) => {
    const source = featured_image
      ? { uri: featured_image.file_uri }
  ...
  renderSentIcon = show => {
    ...
    return (
      <View style={styles.sent_icon}>
        ...
        />
        <Text
          style={{ color: "white", fontFamily: "Quicksand-Bold", fontSize: 14 }}
        >
          SENT
        </Text>
      </View>
  ...
  renderDelete = () => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => this.props.delete(this.props.request.id)}
      >
        <Image
  ...
  render() {
    ...
    return (
      <TouchableOpacity
        style={[styles.card, sent_at && styles.sent]}
        onPress={() => this.props.onPress(this.props.request)}
        activeOpacity={1}
      >
