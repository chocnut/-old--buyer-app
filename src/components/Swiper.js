...
import { StyleSheet, View, ViewPropTypes, Text, PanResponder, TouchableOpacity, Animated, Keyboard } from 'react-native';
            ...
            if (this.props.enableSwipe) {
                ...
                if (Math.abs(gestureState.vy < 1)) {
                    if (gestureState.vx >= 0.5) {
                        if (this.state.activeIndex >= 1)  this.moveUpDown(this.state.activeIndex - 1)
                    }
                    else if (gestureState.vx <= -0.5) {
                        if (this.state.activeIndex <= 2)  this.moveUpDown(this.state.activeIndex + 1)
                    }
                }
            }
        },
    ...
    renderControls () {
        ...
        return (
            <View style={[
                ...
                <TouchableOpacity disabled={!activeIndex} style={{opacity:!activeIndex ? 0 : 1}} onPress={()=>this.moveUpDown(this.state.activeIndex - 1)}>
    ...
    render() {
        ...
        return (
            <View style={[styles.container,containerStyle]} onLayout={this._onLayout.bind(this)} >
                <View style={[styles.sliderContainer,swipeAreaStyle]} {...this._panResponder.panHandlers}>
                    <Animated.View
                        style={[{
                            position: "relative",
                            top: 0,
